...
void VulkanDevice::initializeCommandPool(const QueueFamily& family, const uint32_t threadID, const bool createSecondary)
{
    ...
    if (!threadInfo.commandPools.contains(family.index))
    {
        ...
        if (vkCreateCommandPool(m_vkHandle, &poolInfo, nullptr, &threadInfo.commandPools[family.index].pool) !=
            VK_SUCCESS)
        {
            throw std::runtime_error("Failed to create command pool");
        }
        Logger::print(
            "Created main command pool for thread " + std::to_string(threadID) + " and family " + std::to_string(
                family.index));

        if (createSecondary)
        {
            if (vkCreateCommandPool(m_vkHandle, &poolInfo, nullptr,
                                    &threadInfo.commandPools[family.index].secondaryPool) != VK_SUCCESS)
            {
                throw std::runtime_error("Failed to create secondary command pool");
            }
            Logger::print(
                "Created secondary command pool for thread " + std::to_string(threadID) + " and family " +
                std::to_string(family.index));
        }
    }
}
...
uint32_t VulkanDevice::createCommandBuffer(const QueueFamily& family, const uint32_t threadID, const bool isSecondary)
{
    ...
    Logger::print(
        "Allocated command buffer for thread " + std::to_string(threadID) + " and family " + std::to_string(
            family.index));
...
uint32_t VulkanDevice::getOrCreateCommandBuffer(const QueueFamily& family, const uint32_t threadID,
                                                const bool isSecondary)
{
    for (const VulkanCommandBuffer& buffer : m_commandBuffers[threadID])
        if (buffer.m_familyIndex == family.index && buffer.m_threadID == threadID && buffer.m_isSecondary ==
            isSecondary)
            return buffer.getID();
...
void VulkanDevice::freeCommandBuffer(const uint32_t id, const uint32_t threadID)
{
    for (auto it = m_commandBuffers[threadID].begin(); it != m_commandBuffers[threadID].end(); ++it)
    {
        if (it->m_id == id)
        {
            vkFreeCommandBuffers(m_vkHandle, m_threadCommandInfos[it->m_threadID].commandPools[it->m_familyIndex].pool,
                                 1, &it->m_vkHandle);
...
uint32_t VulkanDevice::createFramebuffer(const VkExtent3D size, const uint32_t renderPass,
...
uint32_t VulkanDevice::createBuffer(const VkDeviceSize size, const VkBufferUsageFlags usage)
{
    ...
    Logger::print(
        "Created buffer with id " + std::to_string(m_buffers.back().getID()) + " and size " + std::to_string(
            m_buffers.back().getSize()));
...
uint32_t VulkanDevice::createImage(const VkImageType type, const VkFormat format, const VkExtent3D extent,
...
void VulkanDevice::configureStagingBuffer(const VkDeviceSize size, const QueueSelection& queue,
                                          const bool forceAllowStagingMemory)
{
    ...
    const std::optional<uint32_t> memoryType = m_memoryAllocator.getMemoryStructure().getStagingMemoryType(
        memRequirements.memoryTypeBits);
    if (memoryType.has_value() && !m_memoryAllocator.isMemoryTypeHidden(memoryType.value()))
    {
        const uint32_t heapIndex = m_physicalDevice.getMemoryProperties().memoryTypes[memRequirements.memoryTypeBits].
            heapIndex;
        const VkDeviceSize heapSize = m_physicalDevice.getMemoryProperties().memoryHeaps[heapIndex].size;
        if (heapSize < size)
        {
            stagingBuffer.allocateFromFlags({
...
void VulkanDevice::dumpStagingBuffer(const uint32_t buffer, const VkDeviceSize size, const VkDeviceSize offset,
                                     const uint32_t threadID)
{
    dumpStagingBuffer(buffer, {{offset, 0, size}}, threadID);
}

void VulkanDevice::dumpStagingBuffer(const uint32_t buffer, const std::vector<VkBufferCopy>& regions,
                                     const uint32_t threadID)
{
    VulkanBuffer& stagingBuffer = getBuffer(m_stagingBufferInfo.stagingBuffer);
    ...
    const QueueFamily transferQueueFamily = m_physicalDevice.getQueueFamilies().getQueueFamily(
        m_stagingBufferInfo.queue.familyIndex);
    VulkanCommandBuffer& commandBuffer = getCommandBuffer(createCommandBuffer(transferQueueFamily, threadID, false),
                                                          threadID);
...
uint32_t VulkanDevice::createRenderPass(const VulkanRenderPassBuilder& builder, const VkRenderPassCreateFlags flags)
{
    ...
    for (const auto& subpass : builder.m_subpasses)
    {
        ...
        subpassInfo.pDepthStencilAttachment = subpass.hasDepthStencilAttachment
    ...
    Logger::print(
        "Created renderpass with id " + std::to_string(m_renderPasses.back().getID()) + ", " +
        std::to_string(builder.m_attachments.size()) + " attachment(s) and " + std::to_string(
            builder.m_subpasses.size()) + " subpass(es)");
...
uint32_t VulkanDevice::createPipelineLayout(const std::vector<uint32_t>& descriptorSetLayouts,
...
uint32_t VulkanDevice::createDescriptorSetLayout(const std::vector<VkDescriptorSetLayoutBinding>& bindings,
...
std::vector<uint32_t> VulkanDevice::createDescriptorSets(const uint32_t pool, const uint32_t layout,
...
{
    const VulkanShader::Result result = VulkanShader::compileFile(filename, VulkanShader::getKindFromStage(stage),
                                                                  VulkanShader::readFile(filename), true);
    ...
    Logger::print(
        "Created shader with id " + std::to_string(m_shaders.back().getID()) + " and stage " +
...
uint32_t VulkanDevice::createPipeline(const VulkanPipelineBuilder& builder, const uint32_t pipelineLayout,
                                      const uint32_t renderPass, const uint32_t subpass)
{
    ...
    for (const auto& commandBuffers : m_commandBuffers | std::views::values)
        for (const VulkanCommandBuffer& buffer : commandBuffers)
            vkFreeCommandBuffers(
                m_vkHandle, m_threadCommandInfos[buffer.m_threadID].commandPools[buffer.m_familyIndex].pool, 1,
                &buffer.m_vkHandle);
