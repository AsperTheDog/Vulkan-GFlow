...
void VulkanCommandBuffer::cmdCopyBuffer(const uint32_t source, const uint32_t destination,
                                        const std::vector<VkBufferCopy>& copyRegions) const
{
    ...
    vkCmdCopyBuffer(m_vkHandle, device.getBuffer(source).m_vkHandle, device.getBuffer(destination).m_vkHandle,
                    static_cast<uint32_t>(copyRegions.size()), copyRegions.data());
}

void VulkanCommandBuffer::cmdPushConstant(const uint32_t layout, const VkShaderStageFlags stageFlags,
                                          const uint32_t offset, const uint32_t size, const void* pValues) const
{
    if (!m_isRecording)
    {
        throw std::runtime_error("Command buffer is not recording");
    ...
    vkCmdPushConstants(m_vkHandle, VulkanContext::getDevice(m_device).getPipelineLayout(layout).m_vkHandle, stageFlags,
                       offset, size, pValues);
}

void VulkanCommandBuffer::submit(const VulkanQueue& queue,
                                 const std::vector<std::pair<uint32_t, VkSemaphoreWaitFlags>>& waitSemaphoreData,
                                 const std::vector<uint32_t>& signalSemaphores, const uint32_t fence) const
{
    if (m_isRecording)
    {
    ...
    vkQueueSubmit(queue.m_vkHandle, 1, &submitInfo,
                  fence != UINT32_MAX ? device.getFence(fence).m_vkHandle : VK_NULL_HANDLE);
}
...
void VulkanCommandBuffer::cmdBeginRenderPass(const uint32_t renderPass, const uint32_t frameBuffer,
...
void VulkanCommandBuffer::cmdPipelineBarrier(const VkPipelineStageFlags srcStageMask,
                                             ...
                                             const std::vector<VkImageMemoryBarrier>& imageMemoryBarriers) const
{
    ...
    vkCmdPipelineBarrier(m_vkHandle, srcStageMask, dstStageMask, dependencyFlags,
...
void VulkanCommandBuffer::cmdBindVertexBuffers(const std::vector<uint32_t>& bufferIDs,
...
void VulkanCommandBuffer::cmdBindIndexBuffer(const uint32_t bufferID, const VkDeviceSize offset,
                                             const VkIndexType indexType) const
{
    ...
    vkCmdBindIndexBuffer(m_vkHandle, VulkanContext::getDevice(m_device).getBuffer(bufferID).m_vkHandle, offset,
                         indexType);
}
...
void VulkanCommandBuffer::cmdDrawIndexed(const uint32_t indexCount, const uint32_t firstIndex,
...
VulkanCommandBuffer::VulkanCommandBuffer(const uint32_t device, const VkCommandBuffer commandBuffer,
                                         const bool isSecondary, const uint32_t familyIndex, const uint32_t threadID)
    : m_vkHandle(commandBuffer), m_isSecondary(isSecondary), m_familyIndex(familyIndex), m_threadID(threadID),
      m_device(device)
{
}
