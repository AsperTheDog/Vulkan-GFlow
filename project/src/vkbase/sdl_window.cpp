...
SDLWindow::SDLWindow(const std::string_view name, const int width, const int height, const int top, const int left,
                     const uint32_t flags)
{
    Logger::print("Initializing SDL window");
...
void SDLWindow::createSwapchain(const uint32_t deviceID, const VkSurfaceFormatKHR desiredFormat)
{
    ...
    const VkSurfaceFormatKHR selectedFormat = VulkanContext::getDevice(deviceID).getGPU().getClosestFormat(
        *this, desiredFormat);
...
uint32_t SDLWindow::acquireNextImage(const uint32_t semaphoreID, const VulkanFence* fence) const
{
    ...
    const VkResult result = vkAcquireNextImageKHR(device.m_vkHandle, m_swapchain.swapchain, UINT64_MAX,
                                                  semaphore.m_vkHandle, fence != nullptr ? fence->m_vkHandle : nullptr,
                                                  &imageIndex);
