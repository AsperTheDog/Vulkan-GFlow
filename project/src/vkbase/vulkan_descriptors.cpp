...
void VulkanDescriptorPool::free()
{
    if (m_vkHandle != VK_NULL_HANDLE)
    {
        ...
        Logger::print(
            "Freeing descriptor pool " + std::to_string(m_id) + (canDescrsBeFreed
                                                                     ? ""
                                                                     : " alongside all associated descriptor sets"));
...
VulkanDescriptorSetLayout::VulkanDescriptorSetLayout(const uint32_t device,
...
void VulkanDescriptorSet::free()
{
    if (m_vkHandle != VK_NULL_HANDLE)
    {
        ...
        vkFreeDescriptorSets(VulkanContext::getDevice(m_device).m_vkHandle,
                             VulkanContext::getDevice(m_device).getDescriptorPool(m_pool).m_vkHandle, 1, &m_vkHandle);
        m_vkHandle = VK_NULL_HANDLE;
    }
}

VulkanDescriptorSet::VulkanDescriptorSet(const uint32_t device, const uint32_t pool,
                                         const VkDescriptorSet descriptorSet)
    : m_vkHandle(descriptorSet), m_device(device), m_pool(pool),
      m_canBeFreed(
          (VulkanContext::getDevice(m_device).getDescriptorPool(pool).m_flags &
