...
void VulkanImage::allocateFromIndex(const uint32_t memoryIndex)
{
    ...
    setBoundMemory(
        VulkanContext::getDevice(m_device).m_memoryAllocator.allocate(requirements.size, requirements.alignment,
                                                                      memoryIndex));
    Logger::popContext();
}
...
void VulkanImage::allocateFromFlags(const VulkanMemoryAllocator::MemoryPropertyPreferences memoryProperties)
{
    Logger::pushContext("Image memory");
    const VkMemoryRequirements requirements = getMemoryRequirements();
    setBoundMemory(VulkanContext::getDevice(m_device).m_memoryAllocator.searchAndAllocate(
        requirements.size, requirements.alignment, memoryProperties, requirements.memoryTypeBits));
    Logger::popContext();
}
...
VkImageView VulkanImage::createImageView(const VkFormat format, const VkImageAspectFlags aspectFlags)
{
    ...
    if (vkCreateImageView(VulkanContext::getDevice(m_device).m_vkHandle, &createInfo, nullptr, &imageView) !=
        VK_SUCCESS)
    {
...
void VulkanImage::transitionLayout(const VkImageLayout layout, const VkImageAspectFlags aspectFlags,
...
VulkanImage::VulkanImage(const uint32_t device, const VkImage vkHandle, const VkExtent3D size, const VkImageType type,
...
void VulkanImage::setBoundMemory(const MemoryChunk::MemoryBlock& memoryRegion)
{
    ...
    Logger::print(
        "Bound memory to image " + std::to_string(m_id) + " with size " + std::to_string(m_memoryRegion.size) +
        " and offset " + std::to_string(m_memoryRegion.offset));
    vkBindImageMemory(VulkanContext::getDevice(m_device).m_vkHandle, m_vkHandle,
                      VulkanContext::getDevice(m_device).getMemoryHandle(m_memoryRegion.chunk), m_memoryRegion.offset);
}
...
void VulkanImage::free()
{
    ...
    Logger::print(
        "Freed image " + std::to_string(m_id) + " with " + std::to_string(m_imageViews.size()) + " image views");
