...
{
    void* data;
    vkMapMemory(VulkanContext::getDevice(m_device).m_vkHandle,
                VulkanContext::getDevice(m_device).getMemoryHandle(m_memoryRegion.chunk),
                m_memoryRegion.offset + offset, size, 0, &data);
    m_mappedData = data;
    return data;
}
...
{
    vkUnmapMemory(VulkanContext::getDevice(m_device).m_vkHandle,
                  VulkanContext::getDevice(m_device).getMemoryHandle(m_memoryRegion.chunk));
...
void VulkanBuffer::setBoundMemory(const MemoryChunk::MemoryBlock& memoryRegion)
{
    ...
    Logger::print(
        "Bound memory to buffer " + std::to_string(m_id) + " with size " + std::to_string(m_memoryRegion.size) +
        " and offset " + std::to_string(m_memoryRegion.offset));
    vkBindBufferMemory(VulkanContext::getDevice(m_device).m_vkHandle, m_vkHandle,
