...
std::string MemoryStructure::toString() const
{
    ...
    for (uint32_t i = 0; i < m_memoryProperties.memoryHeapCount; i++)
    {
        ...
        for (uint32_t j = 0; j < m_memoryProperties.memoryTypeCount; j++)
        {
            ...
            {
                str += "    - Memory Type " + std::to_string(j) + ": " + string_VkMemoryPropertyFlags(
                    m_memoryProperties.memoryTypes[j].propertyFlags) + "\n";
            }
        }
    }
    return str;
...
{
    std::vector<uint32_t> types = getMemoryTypes(
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT | VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
        VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, typeFilter);
...
std::vector<uint32_t> MemoryStructure::getMemoryTypes(const VkMemoryPropertyFlags properties,
                                                      const uint32_t typeFilter) const
{
...
MemoryChunk::MemoryBlock MemoryChunk::allocate(const VkDeviceSize newSize, const VkDeviceSize alignment)
{
    ...
    Logger::print(
        "Allocated block of size " + std::to_string(newSize) + " at offset " + std::to_string(best) + " of memory type "
        + std::to_string(m_memoryType));
...
void MemoryChunk::deallocate(const MemoryBlock& block)
{
    ...
    Logger::print(
        "Deallocated block of size " + std::to_string(block.size) + " at offset " + std::to_string(block.offset) +
        " of memory type " + std::to_string(m_memoryType));
...
void MemoryChunk::defragment()
{
    ...
    for (auto it = m_unallocatedData.begin(); it != m_unallocatedData.end();)
    {
        ...
        if (next != m_unallocatedData.end() && it->first + it->second == next->first)
        {
            ...
            Logger::print(
                "Merged blocks at offsets " + std::to_string(it->first) + " and " + std::to_string(next->first) +
                ", new size: " + std::to_string(it->second));
            mergeCount++;
...
MemoryChunk::MemoryBlock VulkanMemoryAllocator::allocate(const VkDeviceSize size, const VkDeviceSize alignment,
                                                         const uint32_t memoryType)
{
    ...
    Logger::print(
        "Allocated chunk of size " + compactBytes(chunkSize) + " of memory type " + std::to_string(memoryType) +
        " (ID: " + std::to_string(m_memoryChunks.back().getID()) + ")");
    return m_memoryChunks.back().allocate(size, alignment);
}

MemoryChunk::MemoryBlock VulkanMemoryAllocator::searchAndAllocate(const VkDeviceSize size, const VkDeviceSize alignment,
                                                                  const MemoryPropertyPreferences properties,
                                                                  const uint32_t typeFilter, const bool includeHidden)
{
    ...
    for (const auto& type : memoryType)
    {
        ...
        const bool doesMemoryHaveUndesired = m_memoryStructure.doesMemoryContainProperties(
            type, properties.undesiredProperties);
        ...
        const VkDeviceSize remainingSize = getRemainingSize(
            m_memoryStructure.m_memoryProperties.memoryTypes[type].heapIndex);
