...
VulkanRenderPassBuilder& VulkanRenderPassBuilder::addSubpass(const VkPipelineBindPoint bindPoint,
                                                             ...
                                                             const VkSubpassDescriptionFlags flags)
{
    ...
    if (depthCount > 1)
        std::cout << "[RENDER PASS WARNING] Only 1 depth stencil attachment allowed in a subpass, received " <<
            depthCount << " instead\n";

    if ((flags & VK_SUBPASS_DESCRIPTION_SHADER_RESOLVE_BIT_QCOM) == 0 && !subpass.resolveAttachments.empty() && subpass.
        resolveAttachments.size() != subpass.colorAttachments.size())
        std::cout <<
            "[RENDER PASS WARNING] Number of resolve attachments must be equal to the number of color attachments\n";
...
VkAttachmentDescription VulkanRenderPassBuilder::createAttachment(const VkFormat format,
