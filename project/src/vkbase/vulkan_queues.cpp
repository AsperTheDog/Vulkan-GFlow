...
std::string GPUQueueStructure::toString() const
{
    ...
    for (const auto& queueFamily : queueFamilies)
    {
        ...
        result += "  Min Image Transfer Granularity: " +
            std::to_string(queueFamily.properties.minImageTransferGranularity.width) + ", " +
            std::to_string(queueFamily.properties.minImageTransferGranularity.height) + ", " + std::to_string(
                queueFamily.properties.minImageTransferGranularity.depth) + "\n";
    }
    return result;
}
