...
void Engine::createSwapchainFramebuffers(const bool destroy)
{
    for (uint32_t i = 0; i < m_window.getImageCount(); i++)
    {
        ...
        m_framebuffers[i] = VulkanContext::getDevice(m_deviceID).createFramebuffer(
            extent, m_imguiRenderPassID, attachments);
    }
}
...
void Engine::recordCommandBuffer(const uint32_t framebuffer, ImDrawData* im_draw_data) const
{
    ...
    VulkanCommandBuffer& graphicsBuffer = VulkanContext::getDevice(m_deviceID).getCommandBuffer(
        m_graphicsCmdBufferID, 0);
...
void Engine::initImgui() const
{
    ...
    const uint32_t imguiPoolID = device.createDescriptorPool(
        pool_sizes, 1000U * static_cast<uint32_t>(pool_sizes.size()),
    ...
    GUIManager::init(m_window, init_info);
}
