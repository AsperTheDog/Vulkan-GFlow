...
public:
    ...
    uint32_t createFramebuffer(VkExtent3D size, uint32_t renderPass,
    ...
    uint32_t createImage(VkImageType type, VkFormat format, VkExtent3D extent, VkImageUsageFlags usage,
    ...
    uint32_t createPipelineLayout(const std::vector<uint32_t>& descriptorSetLayouts,
    ...
    uint32_t createPipeline(const VulkanPipelineBuilder& builder, uint32_t pipelineLayout, uint32_t renderPass,
                            uint32_t subpass);
    ...
    uint32_t createDescriptorSetLayout(const std::vector<VkDescriptorSetLayoutBinding>& bindings,
