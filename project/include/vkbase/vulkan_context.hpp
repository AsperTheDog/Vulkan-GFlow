...
{
public:
    static void init(uint32_t vulkanApiVersion, bool enableValidationLayers,
                     const std::vector<const char*>& extensions);

    static [[nodiscard]] std::vector<VulkanGPU> getGPUs();

    static uint32_t createDevice(VulkanGPU gpu, const QueueFamilySelector& queues,
                                 const std::vector<const char*>& extensions, const VkPhysicalDeviceFeatures& features);
