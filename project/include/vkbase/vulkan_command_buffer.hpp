...
public:
    ...
    void submit(const VulkanQueue& queue,
                ...
                const std::vector<uint32_t>& signalSemaphores, uint32_t fence = UINT32_MAX) const;
    void reset() const;

    void cmdBeginRenderPass(uint32_t renderPass, uint32_t frameBuffer, VkExtent2D extent,
    ...
    void cmdPushConstant(uint32_t layout, VkShaderStageFlags stageFlags, uint32_t offset, uint32_t size,
...
private:
    VulkanCommandBuffer(uint32_t device, VkCommandBuffer commandBuffer, bool isSecondary, uint32_t familyIndex,
                        uint32_t threadID);
